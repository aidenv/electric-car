(function($) {
    ...
    $(document).ready(function () {
        ...
        $(".case-modal-trigger").on('click', function() {
            ...
            $(".case-modal-" + disputeId).modal("show");
        });
    ...
    function fileDispute ($this)
    {
        ...
        if (parseInt($disputeOrderProductStatus.val().trim()) === 0 || $disputeOrderProductStatus.val().trim() === '') {
        ...
        if ($reasonId === 0) {
            $messageModal.find('.header-content').html('Invalid Reason.');
            $messageModal.find('.detail-content').html('');
            $messageModal.modal('show');
            return false;
        }
        ...
        $.ajax({
            url: Routing.generate('dashboard_resolution_center_add'),
            method: 'post',
            dataType: 'json',
            data: {
    ...
    function getOrderProductByOrder (orderId)
    {
        ...
        $.ajax({
            url: Routing.generate('dashboard_get_order_products'),
            method: 'get',
            dataType: 'json',
            data: {
                orderId: orderId
