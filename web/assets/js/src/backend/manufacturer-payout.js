(function ($) {
    ...
    $(document).ready(function () {
        ...
        $('#sellerContainer').on('click', '.sellerPayoutRow', function () {
            ...
            getMerchantOrderProducts (orderProductIds);
        });
     ...
     */
    function getMerchantOrderProducts (orderProductIds)
    {
     ...
     * @param orderProducts
     */
    function displayProductOrder (orderProducts)
    {
        /**
        ...
        $.each(orderProducts, function (key, orderProduct) {
             ...
             */
            if (parseInt(orderProduct.orderProductStatusId) === parseInt($('#status-item-received').val()) || parseInt(orderProduct.orderProductStatusId) === parseInt($('#status-payout-unheld').val())) {
    ...
    function proceedPayment (orderProductIds, $this)
    {
        ...
        $.ajax({
            url: Routing.generate('yilinker_backend_manufacturer_payout_proceed_payment'),
            method: 'POST',
            dataType: 'json',
            data: formdata,
            processData: false,
