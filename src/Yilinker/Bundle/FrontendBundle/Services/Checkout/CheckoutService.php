...
class CheckoutService
{
    ...
    public function buynow($productId, $unitId, $quantity, $sellerId)
    {
        $this->cartService->updateCart($productId, $unitId, $quantity, 0, $sellerId);
        $itemId = null;
    ...
    public function saveGuestUser($user)
    {
        ...
        $realUser = $tbUser->findOneBy(array(
        ...
        if ($realUser) {
            ...
            $user
                ->setPlainPassword('')
                ->setUserType(User::USER_TYPE_GUEST)
            ;

            if ($this->api) {
                $user->setRegistrationType(User::REGISTRATION_TYPE_MOBILE);
            }
    ...
    {
        $user = $this->getCheckoutUser();
        if ((int) $user->getUserType() === User::USER_TYPE_GUEST) {
            $user->setUserType(User::USER_TYPE_BUYER);
            $user->setIsActive(true);
            $password = $user->createAutoGeneratedPassword();
            ...
            $seller = clone $user;
            $this->em->detach($seller);
            $this->em->persist($seller);
            $seller->setUserType(User::USER_TYPE_SELLER);
            ...
            $accountManager->registerUser($user, false, $user->getIsMobileVerified(), $user->getIsEmailVerified());
            $accountManager->registerUser($seller, false, $seller->getIsMobileVerified(), $seller->getIsEmailVerified());
            $store = $storeService->createStore($seller, Store::STORE_TYPE_RESELLER);
            $store->setStoreNumber($storeService->generateStoreNumber($store));

            $this->em->flush();
            
            $message = $this->container->get("templating")->render(
                'YilinkerCoreBundle:SMS:checkout_autogenerated_password.html.twig', array(
                    'password' => $password,
                )
            );

            /**
             * Please apply country in user entity for the globalization. This will onl be for tempo
             */
            $country = $this->em->getRepository("YilinkerCoreBundle:Country")->findOneByAreaCode("63");
            $smsSender = $this->container->get("yilinker_core.service.sms.sender");
            $smsSender->setMode(SmsSender::SMS_MODE_BROADCAST);
            $smsSender->getService($country->getAreaCode(), null);
    ...
    public function makeOrderProducts($userOrder, $cart)
    {
        ...
        foreach ($cart as $product) {
            ...
            $productWarehouse = $productObj->getProductWarehouse();
            $handlingFee = 0;
            $shippingFee = 0;
            // if logistics is express
            if (!$productWarehouse || $productWarehouse->getLogistics()->getLogisticsId() == 1) {
                ...
                $shippingFee = $productWarehouse->getHandlingFee() * $product['quantity'];
            }
            ...
            $orderProduct->setSeller($tbProduct->getActualSeller($product));
            ...
            $orderProduct->setUserWarehouse($productUnit->getWarehouse());
            ...
            if($productUnit->isInhouseProductUnit()){
                $commission = $productUnit->getCommission();
                $commission = $commission * $product['quantity'];
                $orderProduct->setCommission($commission);
            }
    ...
    public function makeUserOrder($paymentType = PaymentMethod::PAYMENT_METHOD_COD)
    {
        ...
        $userOrder->setIsNewAccount($user->getUserType() == User::USER_TYPE_GUEST);
    ...
    public function getActiveVoucherCode($code, $number_format = false, $entity = true)
    {
        ...
        $isValidVoucher = $voucherCode ? $this->isValidVoucherType($voucher) : false;
        if ($isValidVoucher === false){
            $voucherCode = null;
            $voucher = null;
        }
    ...
    public function continueShoppingURL($userOrder)
    {
        ...
        foreach ($orderProducts as $orderProduct) {
            ...
            if (in_array($seller->getUserId(), array(203953))) {
                $slug = $seller->getStore()->getStoreSlug();

                return $router->generate('store_page_products', compact('slug'));
            }
    ...
    /**
     * Check if cartItem has Flashsales item
     * @return boolean
     */
    public function hasFlashSaleItem()
    {
        $cart = $this->getSelectedOnCart();

        $hasFlashSaleItem = false;
        foreach ($cart as $c) {
            if(array_key_exists($c['unitId'], $c['productUnits'])) {
                foreach ($c['productUnits'][$c['unitId']]['promoInstance'] as $promoInstance) {
                    if ($promoInstance['isEnabled'] && $promoInstance['promoType'] == 4) {
        ...
        return $hasFlashSaleItem;

    }

    /**
     * Check Valid Vouchers by product, productcategory, store
     */
    public function isValidVoucherType($voucher)
    {
        $cart = $this->getSelectedOnCart();
        $validVouchers = $this->validVouchers($voucher);
        ...
        foreach ($cart as $c) {
            ...
            if (count($validVouchers['producs']) > 0 &&
                !in_array($c['id'], $validVouchers['producs'])) {
                array_push($errors['products'], $c['id']);
            }

            if (count($validVouchers['productCategories']) > 0) {
                $countProdCategories = 0;
                foreach ($validVouchers['productCategories'] as $key => $item) {
                    if (($c['productCategoryId'] == $key) || in_array($c['productCategoryId'], $item) ) {
                        $countProdCategories++;
                    }    
                }
                if ($countProdCategories == 0) {
                    array_push($errors['categories'], $c['productCategoryId']);
                }
            } 

            if ($voucher->getIncludeAffiliates() && !$c['isAffiliate']) {
                if (count($validVouchers['stores']) > 0 &&
                 !in_array($c['storeId'], $validVouchers['stores'])) {
                    array_push($errors['stores'], $c['storeId']);
                }
            }
            else if (!$voucher->getIncludeAffiliates()) {
                if (count($validVouchers['stores']) > 0 &&
                 !in_array($c['storeId'], $validVouchers['stores'])) {
                    array_push($errors['stores'], $c['storeId']);
                }   
        ...
        return empty($errors['products']) && empty($errors['categories']) && empty($errors['stores']) ? true : false;
    }

    public function validVouchers($voucher)
    {
        $validVouchers['producs'] = array();
        $validVouchers['productCategories'] = array();
        $validVouchers['stores'] = array();
        
        foreach ($voucher->getVoucherProducts() as $voucherProduct) {
            if (!is_null($voucherProduct->getProduct())) {
                $validVouchers['producs'][] = $voucherProduct->getProduct()->getProductId();
            }
        }

        foreach ($voucher->getVoucherProductCategories() as $voucherProductCategory) {
            if (!is_null($voucherProductCategory->getProductCategory())) {
                $categoryID = $voucherProductCategory->getProductCategory()->getProductCategoryId();
                $tbCategoryNested = $this->em->getRepository('YilinkerCoreBundle:CategoryNestedSet');
                
                $validVouchers['productCategories'][$categoryID] = $tbCategoryNested->getChildrenCategoryIds($categoryID);
            }
        }

        foreach ($voucher->getVoucherStores() as $voucherStore) {
            if (!is_null($voucherStore->getStore())) {
                $validVouchers['stores'][] = $voucherStore->getStore()->getStoreId();
            }
        }
