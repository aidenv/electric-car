...
namespace Yilinker\Bundle\FrontendBundle\Controller\Api\v3;
...
class CheckoutApiController extends Controller
{
     ...
     *         {"name"="location", "dataType"="string", "required"=false, "description"=""},
     *     },
    ...
    public function selectOnCartAction(Request $request)
    {
        try {
            ...
            foreach ($selectedCart as $key=>&$product) {
                $totalAmount = $totalAmount + ($product['productUnits'][$product['unitId']]['discountedPrice'] * $product['quantity']);
                $product['productUnits'] = array_values($product['productUnits']);
                $totalShippingCost = $totalShippingCost + $product['shippingCost'];
            }
        ...
        $country = $entityManager->getRepository("YilinkerCoreBundle:Country")->findOneByCode($countryCode);
        $symbol = $country->getCurrency() ? $country->getCurrency()->getSymbol() : '';
        $total = $totalAmount + $totalShippingCost;

        $cart['items'] =  isset($selectedCart) ? array_values($selectedCart): '';
        $cart['totalShippingCost'] = $totalShippingCost ? $symbol .' '. number_format($totalShippingCost, 2) : 'FREE';
        $cart['totalAmount'] = $symbol .' '. number_format($total, 2);
        $cart['hasFlashSaleItem'] = $this->checkoutService->hasFlashSaleItem();
